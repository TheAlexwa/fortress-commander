...
import { getVeteranModifiers, getVeteranSpecialization, isVeteranChoiceReady } from "./specializations.js";
import {
...
const hitReactionStates=new WeakMap();
const trackedUnitVisuals=new Map();
const trackedEnemyVisuals=new Map();
const deathAnimations=[];
const trackedProjectileVisuals=new Map();
const impactEffects=[];
const battlefieldDecals=[];
...
let visualTrackingWasInWave=false;
...
function nearestOpposingAngle(entity,friendly){
 const opponents=friendly?state.enemies:state.units;
 let nearest=null,best=Infinity;
 for(const candidate of opponents||[]){
  if(!candidate||candidate.hp<=0)continue;
  const distance=Math.hypot(entity.x-candidate.x,entity.y-candidate.y);
  if(distance<best){best=distance;nearest=candidate}
 }
 if(!nearest)return Number.isFinite(entity.attackAngle)?entity.attackAngle+Math.PI:-Math.PI/2;
 return Math.atan2(entity.y-nearest.y,entity.x-nearest.x);
}

function updateHitReaction(entity,previousHp,now,friendly){
 const currentHp=Math.max(0,Number(entity.hp)||0);
 if(!Number.isFinite(previousHp)||currentHp>=previousHp-.01)return;
 const maxHp=Math.max(1,Number(entity.maxHp)||1);
 const damage=Math.max(0,previousHp-currentHp);
 const blocked=friendly?["guard","hero"].includes(entity.key):entity.type==="shield"||entity.shieldProtected===true;
 hitReactionStates.set(entity,{
  start:now,
  duration:blocked?300:240,
  angle:nearestOpposingAngle(entity,friendly),
  strength:Math.max(.22,Math.min(1,damage/maxHp*4.5)),
  blocked
 });
}

function getHitReaction(entity,now=performance.now()){
 const reaction=hitReactionStates.get(entity);
 if(!reaction)return null;
 const progress=(now-reaction.start)/reaction.duration;
 if(progress>=1){hitReactionStates.delete(entity);return null}
 return {...reaction,progress:clamp01(progress),pulse:Math.sin(clamp01(progress)*Math.PI)};
}

function unitVisualSnapshot(unit,now){
 const sprite=unitSprites[unit.key];
 if(!sprite)return null;
 return {kind:"unit",id:unit.uid,key:unit.key,x:unit.x,y:unit.y,hp:unit.hp,maxHp:unit.maxHp,image:sprite.idle,width:sprite.def.width,height:sprite.def.height,offsetY:sprite.def.offsetY||0,angle:Number.isFinite(unit.attackAngle)?unit.attackAngle:0,lastSeen:now,hero:unit.key==="hero"};
}

function enemyVisualSnapshot(enemy,now){
 ...
 const scale=Number.isFinite(enemy.visualScale)?enemy.visualScale:visualClass==="boss"?1.5:visualClass==="special"?1.18:1;
 return {kind:"enemy",id:enemy.eid,type:enemy.type,x:enemy.x,y:enemy.y,hp:enemy.hp,maxHp:enemy.maxHp,image:sprite.image,width:sprite.def.width*scale,height:sprite.def.height*scale,offsetY:(sprite.def.offsetY||0)*scale,angle:Number.isFinite(enemy._facing)?enemy._facing:-Math.PI/2,lastSeen:now,boss:enemy.type==="boss"};
}

function createDeathAnimation(snapshot,now){
 if(!snapshot?.image)return;
 deathAnimations.push({...snapshot,start:now,duration:snapshot.hero?900:snapshot.boss?820:snapshot.kind==="unit"?720:620,fallDirection:(Number(snapshot.id)||1)%2?1:-1});
 if(deathAnimations.length>48)deathAnimations.splice(0,deathAnimations.length-48);
}

function updateCombatVisualTracking(now){
 const combatTransition=state.inWave||visualTrackingWasInWave;
 const currentUnitIds=new Set();
 for(const unit of state.units||[]){
  const id=Number(unit.uid)||0;currentUnitIds.add(id);
  const previous=trackedUnitVisuals.get(id);
  if(previous)updateHitReaction(unit,previous.hp,now,true);
  const snapshot=unitVisualSnapshot(unit,now);if(snapshot)trackedUnitVisuals.set(id,snapshot);
 }
 for(const [id,snapshot] of [...trackedUnitVisuals])if(!currentUnitIds.has(id)){
  if(combatTransition&&now-snapshot.lastSeen<180)createDeathAnimation(snapshot,now);
  trackedUnitVisuals.delete(id);
 }
 const currentEnemyIds=new Set();
 for(const enemy of state.enemies||[]){
  const id=Number(enemy.eid)||0;currentEnemyIds.add(id);
  const previous=trackedEnemyVisuals.get(id);
  if(previous)updateHitReaction(enemy,previous.hp,now,false);
  const snapshot=enemyVisualSnapshot(enemy,now);if(snapshot)trackedEnemyVisuals.set(id,snapshot);
 }
 for(const [id,snapshot] of [...trackedEnemyVisuals])if(!currentEnemyIds.has(id)){
  if(combatTransition&&now-snapshot.lastSeen<180)createDeathAnimation(snapshot,now);
  trackedEnemyVisuals.delete(id);
 }
 visualTrackingWasInWave=!!state.inWave;
 for(let index=deathAnimations.length-1;index>=0;index--)if(now-deathAnimations[index].start>=deathAnimations[index].duration)deathAnimations.splice(index,1);
}

function drawHitReactionEffect(entity,reaction,size=24){
 if(!reaction)return;
 const face=reaction.angle+Math.PI,pulse=reaction.pulse;
 ctx.save();ctx.rotate(face);
 if(reaction.blocked){
  ctx.globalAlpha=(1-reaction.progress)*(.42+.46*pulse);ctx.strokeStyle="#fff1a6";ctx.shadowBlur=12;ctx.shadowColor="#ffd76a";ctx.lineWidth=3.2;ctx.beginPath();ctx.arc(0,0,size,-.72,.72);ctx.stroke();ctx.shadowBlur=0;
  ctx.fillStyle="#fff7c8";for(const offset of [-.22,0,.22]){ctx.save();ctx.rotate(offset);ctx.beginPath();ctx.moveTo(size+2,0);ctx.lineTo(size+8,-2);ctx.lineTo(size+7,2);ctx.closePath();ctx.fill();ctx.restore()}
 }else{
  ctx.translate(size*.58,0);ctx.globalAlpha=(1-reaction.progress)*(.28+.55*pulse);ctx.strokeStyle="#fff5e0";ctx.lineWidth=2;ctx.shadowBlur=9;ctx.shadowColor="#ffcf8a";
  for(let index=0;index<4;index++){ctx.rotate(Math.PI/4);ctx.beginPath();ctx.moveTo(-4,0);ctx.lineTo(7+reaction.strength*5,0);ctx.stroke()}
  ctx.shadowBlur=0;
 }
 ctx.restore();
}
...
function drawDeathAnimations(now){
 for(const death of deathAnimations){
  const progress=clamp01((now-death.start)/death.duration),ease=1-Math.pow(1-progress,3);
  ctx.save();ctx.translate(death.x,death.y+ease*13);ctx.rotate(death.fallDirection*ease*(death.hero?.72:death.boss?.82:.96));ctx.scale(1-progress*.08,1-progress*.2);ctx.globalAlpha=Math.max(0,1-Math.pow(progress,1.35));
  ctx.fillStyle="#05060766";ctx.beginPath();ctx.ellipse(5,death.kind==="enemy"?3:death.height*.37,death.width*.32*(1-progress*.3),Math.max(3,death.height*.07),0,0,TAU);ctx.fill();
  if(death.image.complete&&death.image.naturalWidth){
   if(death.kind==="enemy")ctx.drawImage(death.image,-death.width/2,-death.height+death.offsetY+4,death.width,death.height);
   else ctx.drawImage(death.image,-death.width/2,-death.height/2+death.offsetY,death.width,death.height);
  }
  if(progress<.46){ctx.globalAlpha=(.46-progress)*.28;ctx.fillStyle=death.hero?"#ffd76a":"#c7b18a";for(let i=0;i<4;i++){const a=(i*1.73+(Number(death.id)||0))%TAU,r=8+progress*24;ctx.beginPath();ctx.arc(Math.cos(a)*r,death.height*.3+Math.sin(a)*r*.3,2.2+progress*2,0,TAU);ctx.fill()}}
  ctx.restore();
 }
}

function createProjectileImpact(snapshot,now){
 if(!snapshot)return;
 const heavy=snapshot.ownerKey==="catapult"||snapshot.splash;
 const bolt=snapshot.ownerKey==="crossbow";
 const x=Number.isFinite(snapshot.targetX)?snapshot.targetX:snapshot.x;
 const y=Number.isFinite(snapshot.targetY)?snapshot.targetY:snapshot.y;
 const angle=Number.isFinite(snapshot.angle)?snapshot.angle:0;
 impactEffects.push({x,y,start:now,duration:heavy?720:bolt?360:280,type:heavy?"catapult":bolt?"bolt":"arrow",color:snapshot.color||"#f0d176",angle});
 if(impactEffects.length>36)impactEffects.splice(0,impactEffects.length-36);
 battlefieldDecals.push({x,y,start:now,duration:heavy?9000:5200,type:heavy?"crater":bolt?"bolt":"arrow",angle,seed:snapshot.seed||now});
 if(battlefieldDecals.length>34)battlefieldDecals.splice(0,battlefieldDecals.length-34);
 if(heavy&&cameraEffects&&!REDUCED_MOTION){cameraShakeUntil=Math.max(cameraShakeUntil,now+340);cameraShakeStrength=Math.max(cameraShakeStrength,5.6);cameraShakeSeed=Number(snapshot.seed)||now}
}

function updateProjectileVisualTracking(now){
 const active=new Set();
 for(const projectile of state.projectiles||[]){
  active.add(projectile);
  const target=projectile.target;
  trackedProjectileVisuals.set(projectile,{x:projectile.x,y:projectile.y,targetX:Number(target?.x),targetY:Number(target?.y),angle:target?Math.atan2(target.y-projectile.y,target.x-projectile.x):0,ownerKey:projectile.owner?.key,splash:Number(projectile.splash)||0,color:projectile.color,lastSeen:now,seed:(Number(projectile.owner?.bid)||Number(projectile.owner?.uid)||1)*97+(Number(target?.eid)||1)*31});
 }
 for(const [projectile,snapshot] of [...trackedProjectileVisuals])if(!active.has(projectile)){
  if(now-snapshot.lastSeen<220)createProjectileImpact(snapshot,now);
  trackedProjectileVisuals.delete(projectile);
 }
 for(let index=impactEffects.length-1;index>=0;index--)if(now-impactEffects[index].start>=impactEffects[index].duration)impactEffects.splice(index,1);
 for(let index=battlefieldDecals.length-1;index>=0;index--)if(now-battlefieldDecals[index].start>=battlefieldDecals[index].duration)battlefieldDecals.splice(index,1);
}

function drawBattlefieldDecals(now){
 for(const decal of battlefieldDecals){
  const age=now-decal.start,fade=Math.max(0,Math.min(1,(decal.duration-age)/Math.min(1800,decal.duration*.35)));
  ctx.save();ctx.translate(decal.x,decal.y);ctx.rotate(decal.angle);ctx.globalAlpha=(decal.type==="crater"?.34:.42)*fade;
  if(decal.type==="crater"){
   ctx.fillStyle="#17120f";ctx.beginPath();ctx.ellipse(0,2,16,8,0,0,TAU);ctx.fill();ctx.strokeStyle="#5b4734";ctx.lineWidth=2;ctx.beginPath();ctx.ellipse(0,1,20,10,0,0,TAU);ctx.stroke();
   ctx.strokeStyle="#2c211a";ctx.lineWidth=1.5;for(let i=0;i<5;i++){ctx.save();ctx.rotate(i*TAU/5+(decal.seed%11)*.03);ctx.beginPath();ctx.moveTo(10,0);ctx.lineTo(24+(i%2)*5,0);ctx.stroke();ctx.restore()}
  }else{
   ctx.strokeStyle=decal.type==="bolt"?"#c7c2b5":"#8c6841";ctx.lineWidth=decal.type==="bolt"?2.3:1.6;ctx.beginPath();ctx.moveTo(-7,0);ctx.lineTo(7,0);ctx.stroke();ctx.fillStyle="#d7dbdc";ctx.beginPath();ctx.moveTo(9,0);ctx.lineTo(4,-2.5);ctx.lineTo(4,2.5);ctx.closePath();ctx.fill();
  }
  ctx.restore();
 }
}
...
function drawImpactEffects(now){
 for(const effect of impactEffects){
  ...
  if(effect.type==="catapult"){
   const radius=10+34*progress;ctx.globalAlpha=.48*fade;ctx.strokeStyle="#c9b089";ctx.lineWidth=4*fade+1;ctx.beginPath();ctx.ellipse(0,3,radius,radius*.45,0,0,TAU);ctx.stroke();
   ctx.fillStyle="#80664b";for(let i=0;i<10;i++){const a=i*TAU/10+(effect.start%17)*.01,d=8+28*progress+(i%3)*3;ctx.globalAlpha=(.22+.35*(i%2))*fade;ctx.beginPath();ctx.arc(Math.cos(a)*d,Math.sin(a)*d*.52,2+(i%3),0,TAU);ctx.fill()}
  }else{
   const sparks=effect.type==="bolt"?7:4;ctx.fillStyle=effect.type==="bolt"?"#eef3ef":"#ffe2a2";for(let i=0;i<sparks;i++){const a=-.8+i*(1.6/Math.max(1,sparks-1)),d=4+18*progress+(i%2)*4;ctx.globalAlpha=(.35+.45*(i%2))*fade;ctx.beginPath();ctx.arc(Math.cos(a)*d,Math.sin(a)*d,1.2+(i%2),0,TAU);ctx.fill()}
  }
  ctx.restore();
 }
}

function drawProjectileVisual(projectile){
 const target=projectile.target,angle=target?Math.atan2(target.y-projectile.y,target.x-projectile.x):0;
 const ownerKey=projectile.owner?.key;
 ctx.save();ctx.translate(projectile.x,projectile.y);ctx.rotate(angle);
 if(ownerKey==="catapult"||projectile.splash){
  const spin=performance.now()*.012;ctx.rotate(spin);ctx.shadowBlur=14;ctx.shadowColor=projectile.color;ctx.fillStyle="#71675b";ctx.beginPath();ctx.arc(0,0,Math.max(4,projectile.radius),0,TAU);ctx.fill();ctx.strokeStyle="#b7a58d";ctx.lineWidth=1.5;ctx.stroke();ctx.rotate(-spin);ctx.globalAlpha=.35;ctx.strokeStyle="#d4c2a8";ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-18,0);ctx.lineTo(-5,0);ctx.stroke();
 }else{
  const heavy=ownerKey==="crossbow";const length=heavy?18:15;ctx.globalAlpha=.22;ctx.strokeStyle="#05080a";ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-length+2,4);ctx.lineTo(8,4);ctx.stroke();ctx.globalAlpha=1;ctx.shadowBlur=heavy?12:8;ctx.shadowColor=projectile.color;ctx.strokeStyle=heavy?"#e2d3b0":"#c79a54";ctx.lineWidth=heavy?2.7:2;ctx.beginPath();ctx.moveTo(-length,0);ctx.lineTo(5,0);ctx.stroke();ctx.fillStyle="#eef5f5";ctx.beginPath();ctx.moveTo(9,0);ctx.lineTo(3,-3.4);ctx.lineTo(3,3.4);ctx.closePath();ctx.fill();ctx.fillStyle=heavy?"#76523a":"#8f3e35";ctx.beginPath();ctx.moveTo(-length,0);ctx.lineTo(-length+5,-3);ctx.lineTo(-length+4,0);ctx.lineTo(-length+5,3);ctx.closePath();ctx.fill();ctx.globalAlpha=.25;ctx.strokeStyle=projectile.color;ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(-length-10,0);ctx.lineTo(-length-2,0);ctx.stroke();
 }
 ctx.restore();
}
...
export function renderGameFrame(environment) {
  ({
    ...
    gameOver, zoom, vw, vh, camX, camY, WORLD_W, WORLD_H, CX, CY, WALL_R, TAU, cameraEffects
  } = environment);
  cameraEffects=environment.cameraEffects!==false;
...
function drawFixedInnerWall(radius=FIXED_INNER_WALL_RADIUS){
 ...
 for(const wall of walls){
  ...
  const stone=wall.material==="stone";
  ...
  if(alive)drawFortificationDamageEffect(CX+Math.cos(wall.am)*radius,CY+Math.sin(wall.am)*radius,ratio,(Number(wall.i)||0)+11,stone);
...
function drawMiddleGate(gate,showBlueprint=false){
 ...
 if(gate.built&&gate.hp>0)drawFortificationDamageEffect(0,0,Math.max(0,Math.min(1,gate.hp/Math.max(1,gate.maxHp))),(Number(gate.i)||0)+41,stone,a+Math.PI/2);
...
function drawOuterGate(gate,radius,showBlueprint=false){
 ...
 if(gate.built&&gate.hp>0)drawFortificationDamageEffect(0,0,Math.max(0,Math.min(1,gate.hp/Math.max(1,gate.maxHp))),(Number(gate.i)||0)+61,stone,a+Math.PI/2);
...
function drawOuterWallSegment(wall,radius){
 ...
 if(alive)drawFortificationDamageEffect(CX+Math.cos(wall.am)*radius,CY+Math.sin(wall.am)*radius,ratio,(Number(wall.i)||0)+81,stone);
...
function drawRangeIndicators(){
 ...
 if(selectedOnly){
  if(selected&&selected.kind==="unit"&&selected.hp>0)drawRangeCircle(selected.x,selected.y,selected.range*getVeteranModifiers(selected).rangeMultiplier,"#8bd68d");
  else if(selected&&selected.kind==="building"&&selected.base.kind==="tower"&&selected.hp>0&&((selected.slot.type!=="wall"&&selected.slot.type!=="outer-wall")||wallTowerSpotReady(selected.slot)))drawRangeCircle(selected.slot.x,selected.slot.y,selected.range*getVeteranModifiers(selected).rangeMultiplier,"#72bfff");
  return;
 }
 for(const b of state.buildings){
  if(b.base.kind==="tower"&&b.hp>0&&((b.slot.type!=="wall"&&b.slot.type!=="outer-wall")||wallTowerSpotReady(b.slot)))drawRangeCircle(b.slot.x,b.slot.y,b.range*getVeteranModifiers(b).rangeMultiplier,b.key==="catapult"?"#c9a1ff":"#72bfff");
 }
 for(const u of state.units){
  if(u.hp>0)drawRangeCircle(u.x,u.y,u.range*getVeteranModifiers(u).rangeMultiplier,"#8bd68d");
 }
}
...
function drawBuildingDamageEffects(building,size=34,now=performance.now()){
 const hpRatio=Math.max(0,Math.min(1,building.hp/Math.max(1,building.maxHp)));
 if(hpRatio>=.75)return;
 const seed=(Number(building.bid)||1)*1.731;
 ctx.save();
 ctx.strokeStyle=building.material==="stone"?"#312c29":"#2e1b15";ctx.lineWidth=hpRatio<.35?2.8:2;ctx.lineCap="round";
 ctx.beginPath();ctx.moveTo(-size*.25,-size*.2);ctx.lineTo(-size*.04,-size*.02);ctx.lineTo(-size*.18,size*.28);if(hpRatio<.48){ctx.moveTo(size*.23,-size*.26);ctx.lineTo(size*.05,-size*.04);ctx.lineTo(size*.27,size*.18)}ctx.stroke();
 if(hpRatio<.52){
  const smokeCount=hpRatio<.25?4:2;
  for(let i=0;i<smokeCount;i++){
   const phase=(now*.00032+seed+i*.27)%1,drift=Math.sin(now*.0012+seed+i)*5;
   ctx.globalAlpha=(.20+.22*(1-phase))*(hpRatio<.25?1:.72);ctx.fillStyle=i%2?"#4c4b47":"#2b2c2a";ctx.beginPath();ctx.arc((i-smokeCount/2)*6+drift,-size*.75-phase*24,5+phase*7,0,TAU);ctx.fill();
  }
 }
 if(hpRatio<.26){
  for(let i=0;i<3;i++){const phase=(now*.0011+seed+i*.31)%1;ctx.globalAlpha=.65*(1-phase);ctx.fillStyle=i%2?"#ffce65":"#e86832";ctx.beginPath();ctx.arc(-8+i*8+Math.sin(seed+i)*3,size*.08-phase*18,1.3+(i%2),0,TAU);ctx.fill()}
 }
 ctx.restore();
}

function drawFortificationDamageEffect(x,y,ratio,seed,stone=false,rotation=0){
 if(ratio>=.42)return;
 const now=performance.now();ctx.save();ctx.translate(x,y);ctx.rotate(-rotation);
 const count=ratio<.2?3:1;
 for(let i=0;i<count;i++){
  const phase=(now*.00028+seed*.071+i*.34)%1;
  ctx.globalAlpha=(.18+.2*(1-phase))*(ratio<.2?1:.7);ctx.fillStyle=stone?"#555550":"#3d332b";ctx.beginPath();ctx.arc(Math.sin(now*.001+seed+i)*5,-6-phase*20,4+phase*5,0,TAU);ctx.fill();
 }
 ctx.restore();
}

function drawTowerWeaponAnimation(building,height,offsetY=0){
 const duration=building.key==="catapult"?.48:.34;
 const remaining=Math.max(0,Number(building.attackVisualTime)||0);
 if(remaining<=0)return;
 const progress=Math.max(0,Math.min(1,1-remaining/duration));
 const action=Math.sin(progress*Math.PI);
 const angle=Number.isFinite(building.attackAngle)?building.attackAngle:-Math.PI/2;
 const y=-height/2+offsetY+(building.key==="catapult"?38:31);
 ctx.save();ctx.translate(0,y);ctx.rotate(angle);ctx.lineCap="round";
 if(building.key==="archer"){
  const draw=1-Math.min(1,progress*1.7),bend=10+draw*7;ctx.strokeStyle="#d09b58";ctx.lineWidth=2.6;ctx.beginPath();ctx.arc(0,0,bend,-1.15,1.15);ctx.stroke();ctx.strokeStyle="#f4e6c6";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(Math.cos(-1.15)*bend,Math.sin(-1.15)*bend);ctx.lineTo(-6*draw,0);ctx.lineTo(Math.cos(1.15)*bend,Math.sin(1.15)*bend);ctx.stroke();ctx.strokeStyle="#e8d39c";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(-5*draw,0);ctx.lineTo(18+action*4,0);ctx.stroke();
 }else if(building.key==="crossbow"){
  const recoil=action*5;ctx.strokeStyle="#684321";ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(-16+recoil,0);ctx.lineTo(16+recoil,0);ctx.moveTo(-2+recoil,-9);ctx.lineTo(18+recoil,0);ctx.stroke();ctx.strokeStyle="#e8dfc8";ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(-16+recoil,0);ctx.quadraticCurveTo(0+recoil,-12,16+recoil,0);ctx.stroke();
 }else{
  const armAngle=-1.05+progress*1.55;ctx.rotate(armAngle);ctx.strokeStyle="#6b4528";ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(-8,0);ctx.lineTo(20,0);ctx.stroke();ctx.fillStyle="#77716a";ctx.beginPath();ctx.arc(22,0,5,0,TAU);ctx.fill();ctx.strokeStyle="#d2c1a1";ctx.lineWidth=1.5;ctx.stroke();
 }
 if(progress>.35&&progress<.72){ctx.globalAlpha=.7*(1-Math.abs(progress-.52)/.2);ctx.fillStyle=building.key==="catapult"?"#d6b47d":"#fff2b5";ctx.shadowBlur=12;ctx.shadowColor=ctx.fillStyle;ctx.beginPath();ctx.arc(20,0,building.key==="catapult"?5:3,0,TAU);ctx.fill();ctx.shadowBlur=0}
 ctx.restore();
}
...
function drawTowerBuildingSprite(building){
 ...
 ctx.save();ctx.translate(-Math.cos(angle)*recoil*(building.key==="catapult"?3.6:1.8),-Math.sin(angle)*recoil*(building.key==="catapult"?1.8:.8));ctx.rotate((building.key==="catapult"?-.012:.006)*recoil);ctx.drawImage(sprite.image,-width/2,-height/2+offsetY,width,height);ctx.restore();
 drawTowerWeaponAnimation(building,height,offsetY);
...
function drawBuildings(){
 for(const b of state.buildings){
  ...
  if(isTower){
   ...
   drawBuildingDamageEffects(b,40);
   if(lv>=2){ctx.fillStyle="#d7b45d";ctx.beginPath();ctx.arc(0,-58,3+Math.min(5,lv),0,TAU);ctx.fill()}
   const towerVeteran=getVeteranSpecialization(b);if(towerVeteran){ctx.fillStyle="#142018e8";ctx.strokeStyle="#f1cd70";ctx.lineWidth=1.4;ctx.beginPath();ctx.arc(-24,-46,10,0,TAU);ctx.fill();ctx.stroke();ctx.fillStyle="#fff0b5";ctx.font="bold 12px system-ui";ctx.textAlign="center";ctx.fillText(towerVeteran.icon,-24,-42)}
   const hpRatio=Math.max(0,Math.min(1,b.hp/b.maxHp));
   const showTowerHp=selected===b||hpRatio<.999;
   const towerVeteranReady=isVeteranChoiceReady(b);
   const showTowerXp=selected===b||(b.pendingUpgrades||0)>0||towerVeteranReady;
   if(showTowerHp){
   ...
   if(showTowerXp){
    ...
    ctx.fillStyle=towerVeteranReady?"#f1c85f":(b.pendingUpgrades||0)>0?"#6bd3ff":"#348de4";ctx.fillRect(-25,xpY+1,50*txp,4);
    ctx.strokeStyle="#9edcff66";ctx.lineWidth=1;ctx.strokeRect(-26,xpY,52,6);
    if((b.pendingUpgrades||0)>0||towerVeteranReady){ctx.fillStyle=towerVeteranReady?"#fff0ad":"#e2f8ff";ctx.font="bold 10px sans-serif";ctx.textAlign="center";ctx.fillText(towerVeteranReady?"★":"▲",0,xpY+16);}
   }
  ...
  if(!isTower)drawBuildingDamageEffects(b,34);
...
function drawUnits(){
 ...
 const heroAbilityActive=!!hero&&(Number(hero.heroAbilityTime)||0)>0;
 for(const u of state.units){
  ctx.save();ctx.translate(u.x,u.y);const auto=u.controlMode==="auto",veteranReady=isVeteranChoiceReady(u),ready=(u.pendingUpgrades||0)>0||veteranReady,lv=u.expLevel||1,bob=Math.sin(performance.now()*.0032+(u.uid||0)*1.71)*.42;ctx.translate(0,bob);
  const heroAuraRadius=hero?155*getVeteranModifiers(hero).auraRadiusMultiplier:155;
  const heroAura=hero&&u!==hero&&Math.hypot(u.x-hero.x,u.y-hero.y)<=heroAuraRadius;
  const heroAbilityBuff=heroAbilityActive&&heroAura;
  if(u.key==="hero"){
   ctx.save();const pulse=Math.sin(performance.now()*.006),auraRadius=155*getVeteranModifiers(u).auraRadiusMultiplier,auraScale=auraRadius/155;ctx.globalAlpha=heroAbilityActive ? .29+.08*pulse : .17+.05*Math.sin(performance.now()*.004);ctx.fillStyle=heroAbilityActive?"#ffbd3d":"#ffd35a";ctx.beginPath();ctx.arc(0,0,auraRadius,0,TAU);ctx.fill();ctx.globalAlpha=heroAbilityActive ? .95 : .72;ctx.strokeStyle=heroAbilityActive?"#fff4b0":"#ffe99a";ctx.lineWidth=heroAbilityActive?4:2;ctx.setLineDash(heroAbilityActive?[16,7]:[9,8]);ctx.lineDashOffset=-performance.now()*.025;ctx.beginPath();ctx.arc(0,0,auraRadius,0,TAU);ctx.stroke();ctx.setLineDash([]);
   if(heroAbilityActive){ctx.globalAlpha=.72;ctx.strokeStyle="#b72437";ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,0,(136+7*pulse)*auraScale,0,TAU);ctx.stroke();for(let i=0;i<12;i++){const a=i*TAU/12+performance.now()*.0012;ctx.save();ctx.rotate(a);ctx.fillStyle=i%2?"#ffe27b":"#b72437";ctx.beginPath();ctx.moveTo(0,-174*auraScale);ctx.lineTo(6,-157*auraScale);ctx.lineTo(-6,-157*auraScale);ctx.closePath();ctx.fill();ctx.restore()}}
   ctx.restore();
  }
  if(heroAura){ctx.globalAlpha=heroAbilityBuff ? .34 : .18;ctx.fillStyle=heroAbilityBuff?"#ffb83f":"#ffd45c";ctx.beginPath();ctx.arc(0,2,heroAbilityBuff?27:23,0,TAU);ctx.fill();if(heroAbilityBuff){ctx.globalAlpha=.9;ctx.strokeStyle="#fff0a0";ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,2,25+Math.sin(performance.now()*.009+(u.uid||0))*3,0,TAU);ctx.stroke()}ctx.globalAlpha=1}
  ...
  if(ready){ctx.strokeStyle=veteranReady?"#f1ca62":"#63caff";ctx.shadowBlur=22;ctx.shadowColor=veteranReady?"#e6b94f":"#5ac7ff";ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,0,27+Math.sin(performance.now()*.007)*2,0,TAU);ctx.stroke();ctx.shadowBlur=0}
  if(selected===u){
   if(u.key==="hero"){
  ...
  if(u.key==="hero"){
   ctx.fillStyle=heroAbilityActive?"#7b1725ef":"#2a0c12e8";ctx.strokeStyle=heroAbilityActive?"#fff0a0":"#f1c657";ctx.lineWidth=heroAbilityActive?2.3:1.5;ctx.beginPath();ctx.roundRect(-31,34,62,15,6);ctx.fill();ctx.stroke();
   ctx.fillStyle="#fff1b0";ctx.font="bold 8px Georgia,serif";ctx.textAlign="center";ctx.fillText(heroAbilityActive?"RUF AKTIV":"ANDREAS",0,44);
  ...
  const unitVeteran=getVeteranSpecialization(u);if(unitVeteran){const vy=u.key==="hero"?22:-24;ctx.fillStyle="#142018e8";ctx.strokeStyle="#f1cd70";ctx.lineWidth=1.3;ctx.beginPath();ctx.arc(-22,vy,9,0,TAU);ctx.fill();ctx.stroke();ctx.fillStyle="#fff0b5";ctx.font="bold 11px system-ui";ctx.textAlign="center";ctx.fillText(unitVeteran.icon,-22,vy+4)}
  ...
  if(showUnitXp){const xpY=showUnitHp?hpY+9:hpY;const xpRatio=Math.max(0,Math.min(1,(u.xp||0)/(u.xpMax||65)));ctx.fillStyle="#07101e";ctx.fillRect(-barW/2,xpY,barW,6);ctx.fillStyle=veteranReady?"#f1c85f":ready?"#69d0ff":"#348de4";ctx.fillRect(-barInner/2,xpY+1,barInner*xpRatio,4);ctx.strokeStyle=u.key==="hero"?"#ffe39888":"#a7dfff55";ctx.strokeRect(-barW/2,xpY,barW,6);if(ready){ctx.fillStyle=veteranReady?"#fff0ad":"#dff7ff";ctx.font="bold 10px sans-serif";ctx.fillText(veteranReady?"★":"▲",0,hpY-3)}}ctx.restore();
 }
}
...
function getUnitAttackAnimation(unit){
 const rate=Math.max(.01,unit.rate||0),cooldown=unit.attackCd||0,duration=Math.min(.38,Math.max(.2,rate*.48)),elapsed=rate-cooldown;
 const active=cooldown>0&&elapsed>=0&&elapsed<=duration;
 return {active,progress:active?clamp01(elapsed/duration):0,angle:Number.isFinite(unit.attackAngle)?unit.attackAngle:0,variant:Number(unit.attackVisualVariant)||0};
}

function drawUnitFootDust(motion,unit,height){
 if(!motion.moving)return;
 const footfall=Math.max(0,Math.sin(motion.phase*2));if(footfall<.35)return;
 ctx.save();ctx.globalAlpha=.04+.1*footfall;ctx.fillStyle="#d8c59d";const y=height*.39;ctx.beginPath();ctx.ellipse(-7,y,7+footfall*3,2.2,0,0,TAU);ctx.fill();ctx.beginPath();ctx.ellipse(6,y+1,5+footfall*2,1.8,0,0,TAU);ctx.fill();ctx.restore();
}

function drawArcherAttackEffect(animation,unit){
 const p=animation.progress,release=clamp01(p/.24),recover=clamp01((p-.24)/.76),pull=1-release;
 ctx.save();ctx.rotate(animation.angle);ctx.translate(8,-4);ctx.lineCap="round";
 ctx.strokeStyle="#7b4c27";ctx.lineWidth=2.7;ctx.beginPath();ctx.arc(1,0,11,-1.25,1.25);ctx.stroke();
 const stringX=-8*pull+2*recover;ctx.strokeStyle="#f1e6c8";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(4,-10.5);ctx.lineTo(stringX,0);ctx.lineTo(4,10.5);ctx.stroke();
 ctx.strokeStyle="#d8bb78";ctx.lineWidth=1.8;ctx.beginPath();ctx.moveTo(stringX-2,0);ctx.lineTo(18+release*18,0);ctx.stroke();ctx.fillStyle="#f2f6f7";ctx.beginPath();ctx.moveTo(22+release*18,0);ctx.lineTo(16+release*18,-3);ctx.lineTo(16+release*18,3);ctx.closePath();ctx.fill();
 if(p<.36){ctx.globalAlpha=(1-p/.36)*.65;ctx.strokeStyle="#dff6ff";ctx.shadowBlur=9;ctx.shadowColor="#dff6ff";ctx.beginPath();ctx.moveTo(18+release*18,0);ctx.lineTo(29+release*24,0);ctx.stroke();ctx.shadowBlur=0}
 ctx.restore();
}

function drawGuardAttackEffect(animation,unit){
 const p=animation.progress,swing=Math.sin(p*Math.PI),direction=animation.variant%2?1:-1,hero=unit.key==="hero",radius=hero?38:30;
 ctx.save();ctx.rotate(animation.angle);ctx.scale(1,direction);ctx.globalAlpha=.18+.68*swing;ctx.strokeStyle=hero?"#ffd75f":"#fff0b0";ctx.shadowBlur=hero?16:8;ctx.shadowColor=hero?"#ffbf38":"#fff0b0";ctx.lineCap="round";ctx.lineWidth=hero?7:5;ctx.beginPath();ctx.arc(0,0,radius,-1.08,-1.08+1.95*p);ctx.stroke();ctx.globalAlpha=.42*swing;ctx.strokeStyle="#ffffff";ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,radius+4,-1.02,-1.02+1.82*p);ctx.stroke();ctx.shadowBlur=0;ctx.restore();
 if(hero&&p>.3&&p<.75){ctx.save();ctx.globalAlpha=.22*swing;ctx.strokeStyle="#ffd75f";ctx.lineWidth=3;ctx.beginPath();ctx.ellipse(Math.cos(animation.angle)*8,18,28+12*swing,8+3*swing,0,0,TAU);ctx.stroke();ctx.restore()}
}

function drawHeroAbilityWeaponGlow(unit){
 if(unit.key!=="hero"||(Number(unit.heroAbilityTime)||0)<=0)return;
...
function drawEnemySprite(enemy, now){
 ...
 const attackProgress=attackAnim>0?1-attackAnim:0;
 const attackEase=attackAnim>0?1-Math.pow(1-attackAnim,2):0;
 const hit=getHitReaction(enemy);
 ...
 ctx.save();
 ctx.translate(sway+lungeX+(hit?Math.cos(hit.angle)*hit.pulse*(2.5+hit.strength*4):0),-bob+2+lungeY+(hit?Math.sin(hit.angle)*hit.pulse*2:0));
 ctx.rotate(tilt+(hit?Math.sin(hit.angle)*hit.pulse*.045:0));
 ctx.drawImage(sprite.image,-drawW/2,-drawH+offsetY+4,drawW,drawH);
 if(hit){ctx.save();ctx.globalCompositeOperation="screen";ctx.globalAlpha=(1-hit.progress)*(.14+.32*hit.pulse);ctx.drawImage(sprite.image,-drawW/2,-drawH+offsetY+4,drawW,drawH);ctx.restore()}
 ctx.restore();
 if(attackAnim>0){ctx.save();ctx.rotate(facing);const swing=Math.sin(attackProgress*Math.PI);ctx.globalAlpha=.18+.48*swing;ctx.strokeStyle=enemy.type==="spear"?"#d9e3d2":"#ffd9a2";ctx.shadowBlur=8;ctx.shadowColor=ctx.strokeStyle;ctx.lineCap="round";ctx.lineWidth=enemy.type==="boss"?6:enemy.type==="spear"?3:4;if(enemy.type==="spear"){ctx.beginPath();ctx.moveTo(5,0);ctx.lineTo(18+20*attackProgress,0);ctx.stroke()}else{ctx.beginPath();ctx.arc(0,0,Math.max(24,drawW*.55),-.95,-.95+1.7*attackProgress);ctx.stroke()}ctx.restore()}
 drawHitReactionEffect(enemy,hit,Math.max(22,drawW*.42));
...
function draw(){
 const frameNow=performance.now();updateCombatVisualTracking(frameNow);updateProjectileVisualTracking(frameNow);
 ...
 if(cameraEffects&&!REDUCED_MOTION&&frameNow<cameraShakeUntil){const remaining=(cameraShakeUntil-frameNow)/340,amplitude=cameraShakeStrength*Math.max(0,Math.min(1,remaining));shakeX=Math.sin(frameNow*.071+cameraShakeSeed)*amplitude;shakeY=Math.cos(frameNow*.093+cameraShakeSeed*.7)*amplitude*.65}else if(frameNow>=cameraShakeUntil)cameraShakeStrength=0;
 ctx.save();ctx.translate(shakeX,shakeY);ctx.translate(vw/2,vh/2);ctx.scale(zoom,zoom);ctx.translate(-camX,-camY);
 drawGround();drawPaths();drawWorldDetails();drawBattlefieldDecals(frameNow);drawSiegeCamps();drawCastle();drawFutureFortressLayout();drawSlots();drawRangeIndicators();drawBuildings();drawUnits();drawCraftsmen();
 for(const projectile of state.projectiles)drawProjectileVisual(projectile);
 drawImpactEffects(frameNow);drawEnemies();drawDeathAnimations(frameNow);drawSelectionMarker();for(const p of state.particles){ctx.globalAlpha=Math.min(1,p.life*3);ctx.fillStyle=p.color;ctx.beginPath();ctx.arc(p.x,p.y,p.size,0,TAU);ctx.fill()}ctx.globalAlpha=1;ctx.restore();
